...
{
        # cpu total
        system("gmetric -x 300 --name=cpu_percent_$user --value=$users->{$user}{cpu} --type=float --units=\%cpu");
        
        # mem total (only reported on linux)
        if ( $uname =~ /Linux/ )
        {
                system("gmetric -x 300 --name=mem_percent_$user --value=$users->{$user}{mem} --type=float --units=\%mem");
        }
        
        # vsz total
        system("gmetric -x 300 --name=mem_vsz_kb_$user --value=$users->{$user}{vsz} --type=float --units=kilobytes");

        # cputime total
        system("gmetric -x 300 --name=cpu_total_time_sec_$user --value=$users->{$user}{time} --type=float --units=seconds");
        
        # processes total
        system("gmetric -x 300 --name=procs_total_$user --value=$users->{$user}{procs} --type=float --units=processes");
