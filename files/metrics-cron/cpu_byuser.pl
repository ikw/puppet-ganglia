...
foreach my $user ( keys %$users ) {
	...
	# cpu total
	system("$gmetric --name=cpu_percent_$user --value=".int($users->{$user}{cpu})." --units=\%cpu");

	# mem percent
	system("$gmetric --name=mem_percent_$user --value=".int($users->{$user}{mem})." --units=\%mem");

	# vsz total
	system("$gmetric --name=mem_vsz_kb_$user --value=".int($users->{$user}{vsz})." --units=kilobytes");

	# cputime total
	system("$gmetric --name=cpu_total_time_sec_$user --value=".int($users->{$user}{time})." --units=seconds");

	# processes total
	system("$gmetric --name=procs_total_$user --value=".int($users->{$user}{procs})." --units=processes");
}
